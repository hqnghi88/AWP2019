...
global {
	...
	int nb_of_people<-100 parameter:true;
	...
	float road_density<-8.0;
	
	// Parameters of the strategy
	float time_after_last_stage <-5#mn parameter:true;
...
//	int time_before_hazard;
	...
	float flood_front_speed <-5#m/#mn;
	...
	reflex stop_simu when:inhabitant all_match (each.saved or each.drowned) {
		save [the_alert_strategy,time_before_hazard,casualties]  type: "csv" to: "resultsSim.csv" rewrite: false;
		do pause;
	}
	...
	//	 layout horizontal([0::5000,1::5000,2::5000]) tabs:true editors: false;
		...
		display chart_pie {
			chart "people" type:pie {
				...
				data "evacuees " value: length(inhabitant);		
				data "saved " value: nb_of_people - length(inhabitant) - casualties;	
			}
		  }	
...
experiment exp_batch type: batch repeat: 1 until: inhabitant all_match (each.saved or each.drowned) {
	
	parameter "Alert Strategy" var:the_alert_strategy init:"DEFAULT" among:["STAGE","SPATIAL","DEFAULT"] category: "Strategy";
	parameter "Time before hazard" var:time_before_hazard init:5#mn min:5#mn max:20#mn step: 5#mn category: "Hazard";
